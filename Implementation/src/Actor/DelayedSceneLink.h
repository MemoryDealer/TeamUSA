...
namespace teamusa {
    ...
    ///
    /// \class DelayedSceneLink
    /// \brief Allows the player to transition between scenes, waits until a certain
    ///     number of steps have passed once it's clicked to transition.
    class DelayedSceneLink : public BaseActor {
    ...
    public:
        ...
        // Constructor.
        explicit DelayedSceneLink( Region region, const int scene_ID, const std::string &itemRequired_Text, const int item_ID = -1, const int delay = 0 );

        // Destructor.
        virtual ~DelayedSceneLink( void ) override; // Use "override" keyword to remind yourself/others that this is overriding a parent class method (not necessary).
